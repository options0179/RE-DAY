...
export default function HistoryPage() {
  ...
  return <main className="page-shell narrow"><p className="eyebrow">history</p><h1>내가 지나온 하루</h1><p className="intro">simulations 컬렉션에서 Firebase로 불러온 기록입니다.</p>{error && <p role="alert">{error}</p>}<div className="card-list">{simulations.map((simulation) => <article className="choice-card" key={simulation.sessionId}><div><p className="eyebrow">{new Date(simulation.updatedAt).toLocaleString("ko-KR")}</p><h2>{simulation.jobId}</h2><p>{simulation.status}</p></div><Link className="secondary-button" href={`/simulation/${simulation.sessionId}`}>이어하기</Link></article>)}</div></main>;
}
